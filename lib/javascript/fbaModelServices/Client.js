...
function fbaModelServices(url, auth, auth_cb) {
    ...
    this.find_paths = function(input)
    {
        var resp = json_call_ajax_sync("fbaModelServices.find_paths", [input]);

        return resp[0];
    }

    this.find_paths_async = function(input, _callback, _error_callback)
    {
        json_call_ajax_async("fbaModelServices.find_paths", [input], 1, _callback, _error_callback)
    }
