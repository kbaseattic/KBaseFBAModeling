package Bio::KBase::fbaModel::CLI::Service;
...
sub call_method {
    ...
    my $ctx = Bio::KBase::fbaModel::CLI::ServiceContext->new(client_ip => $self->_plack_req->address);
    
    my $args = $data->{arguments};

        # Service fbaModelCLI does not require authentication.
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service fbaModelCLI.");
    }
...
package Bio::KBase::fbaModel::CLI::ServiceContext;

use strict;

=head1 NAME

Bio::KBase::fbaModel::CLI::ServiceContext
