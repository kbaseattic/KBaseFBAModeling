########################################################################
# Bio::KBase::ObjectAPI::Biochemistry - This is the moose object corresponding to the Biochemistry object
...
########################################################################
use strict;
use Bio::KBase::ObjectAPI::DB::Biochemistry;
use Bio::KBase::ObjectAPI::BiochemistryStructures;
package Bio::KBase::ObjectAPI::Biochemistry;
use Moose;
use Bio::KBase::ObjectAPI::Util::utilities;
use ModelSEED::Configuration;
use namespace::autoclean;
extends 'Bio::KBase::ObjectAPI::DB::Biochemistry';
#***********************************************************************************************************
...
sub printDBFiles {
    my $self = shift;
    my $args = Bio::KBase::ObjectAPI::Util::utilities::args([],{
        forceprint => 0,
...
=head3 makeDBModel
Definition:
	Bio::KBase::ObjectAPI::ModelReaction = Bio::KBase::ObjectAPI::Biochemistry->makeDBModel({
		balancedOnly => 1,
...
sub makeDBModel {
	...
	my $mdl = Bio::KBase::ObjectAPI::Model->new({
		id => $self->name().".model",
		version => 0,
		type => "dbmodel",
...
Definition:
	string = Bio::KBase::ObjectAPI::Biochemistry->export({
...
Definition:
	void Bio::KBase::ObjectAPI::Biochemistry->findCreateEquivalentCompartment({
		compartment => Bio::KBase::ObjectAPI::Compartment(REQ),
		create => 0/1(1)
	});
Description:
...
sub findCreateEquivalentCompartment {
    my $self = shift;
	my $args = Bio::KBase::ObjectAPI::Util::utilities::args(["compartment"], {create => 1}, @_);
...
Definition:
	void Bio::KBase::ObjectAPI::Biochemistry->findCreateEquivalentCompound({
		compound => Bio::KBase::ObjectAPI::Compound(REQ),
		create => 0/1(1)
	});
Description:
...
sub findCreateEquivalentCompound {
    my $self = shift;
	my $args = Bio::KBase::ObjectAPI::Util::utilities::args(["compound"], {create => 1}, @_);
...
Definition:
	void Bio::KBase::ObjectAPI::Biochemistry->findCreateEquivalentReaction({
		reaction => Bio::KBase::ObjectAPI::Reaction(REQ),
		create => 0/1(1)
	});
Description:
...
sub findCreateEquivalentReaction {
    my $self = shift;
	my $args = Bio::KBase::ObjectAPI::Util::utilities::args(["reaction"], {create => 1}, @_);
...
=head3 validate
Definition:
	void Bio::KBase::ObjectAPI::Biochemistry->validate();
Description:
...
Definition:
	void Bio::KBase::ObjectAPI::Biochemistry->findReactionsWithReagent();
Description:
...
Definition:
	Bio::KBase::ObjectAPI::Compartment = Bio::KBase::ObjectAPI::Biochemistry->addCompartmentFromHash({[]});
Description:
...
sub addCompartmentFromHash {
    my ($self,$arguments) = @_;
    $arguments = Bio::KBase::ObjectAPI::Util::utilities::args(["name","id"],{ hierarchy=>3 }, $arguments);

    #check to see if compartment doesn't already exist
    my $cpt = $self->queryObject("compartments",{name => $arguments->{name}});
    if (defined($cpt)) {
	Bio::KBase::ObjectAPI::Util::utilities::verbose("Compartment found with matching name ".$arguments->{name});
	return $cpt;
    }

    Bio::KBase::ObjectAPI::Util::utilities::verbose("Creating compartment ".$arguments->{name}." with id: ".$arguments->{id});
...
Definition:
	Bio::KBase::ObjectAPI::Cue = Bio::KBase::ObjectAPI::Biochemistry->addCueFromHash({[]});
Description:
...
sub addCueFromHash {
    my ($self,$arguments) = @_;
    $arguments = Bio::KBase::ObjectAPI::Util::utilities::args(["name"],{ energy=>[10000000], error=>[10000000], charge=>[10000000] }, $arguments);

    #check to see if cue doesn't already exist
    my $cue = $self->queryObject("cues",{name => $arguments->{name}});
    if (defined($cue)) {
	Bio::KBase::ObjectAPI::Util::utilities::verbose("Cue found with matching name ".$arguments->{name});
	return $cue;
    }

    Bio::KBase::ObjectAPI::Util::utilities::verbose("Creating cue ".$arguments->{name});
    $cue = $self->add("cues",{
...
Definition:
	Bio::KBase::ObjectAPI::Compound = Bio::KBase::ObjectAPI::Biochemistry->addCompoundFromHash({[]});
Description:
...
sub addCompoundFromHash {
    my ($self,$arguments) = @_;
    $arguments = Bio::KBase::ObjectAPI::Util::utilities::args(["names","id"],{
    ...
    if (defined($cpd)) {
	Bio::KBase::ObjectAPI::Util::utilities::verbose("Compound found with matching id ".$arguments->{id}->[0]." for namespace ".$arguments->{namespace});
	...
	if (defined($cpd)) {
	    Bio::KBase::ObjectAPI::Util::utilities::verbose("Compound found with matching id ".$matchingId." for namespace ".$aliasType);
	    $self->addAlias({ attribute => "compounds",
	...
	if(defined($cpd)){
	    Bio::KBase::ObjectAPI::Util::utilities::verbose("Proton found: ".$arguments->{id}->[0].":".join("|",@{$arguments->{names}}));
	    $self->addAlias({ attribute => "compounds",
			      aliasName => $arguments->{namespace},
			      alias => $arguments->{id}->[0],
			      uuid => $cpd->uuid()
	...
	foreach my $name (@{$arguments->{names}}) {
	    ...
	    my $searchname = Bio::KBase::ObjectAPI::Compound::nameToSearchname($name);
	    ...
	    if (defined($cpd)){
		Bio::KBase::ObjectAPI::Util::utilities::verbose("Compound (".$arguments->{id}->[0].") matched based on name ".$name);
    ...
    Bio::KBase::ObjectAPI::Util::utilities::verbose("Creating compound ".$arguments->{id}->[0]);
...
Definition:
	Bio::KBase::ObjectAPI::Compound = Bio::KBase::ObjectAPI::Biochemistry->addReactionFromHash({[]});
Description:
...
sub addReactionFromHash {
    my ($self,$arguments) = @_;
	$arguments = Bio::KBase::ObjectAPI::Util::utilities::args(["equation","id"], {
	    names => undef,
	...
	if (defined($rxn)) {
		Bio::KBase::ObjectAPI::Util::utilities::verbose("Reaction found with matching id ".$arguments->{id}->[0]." for namespace ".$arguments->{reactionIDaliasType});
		if($arguments->{addmergealias}){
		    foreach my $aliasType (@{$arguments->{mergeto}}){
	...
	# Checking for id uniqueness within scope of another aliasType, if passed
	    ...
	    if( defined($rxn) ){
			Bio::KBase::ObjectAPI::Util::utilities::verbose("Reaction found with matching id ".$arguments->{id}->[0]." for namespace ".$aliasType);
	...
	$rxn = Bio::KBase::ObjectAPI::Reaction->new({
		name => $arguments->{names}->[0],
		abbreviation => $arguments->{abbreviation}->[0],
	...
	if(!$rxn->loadFromEquation({
	    ...
	    Bio::KBase::ObjectAPI::Util::utilities::verbose("Reaction ".$arguments->{id}->[0]." was rejected");
	    return undef;
	}else{
	    #Bio::KBase::ObjectAPI::Util::utilities::verbose("Reaction ".$arguments->{id}->[0]." passed: ".$rxn->equationCode());
	}
	...
	if (defined($searchRxn)) {
	    ...
	    Bio::KBase::ObjectAPI::Util::utilities::verbose("Reaction ".$alias." (".$aliasSetName.") found with matching equation for Reaction ".$arguments->{id}->[0]);
	    $self->addAlias({ attribute => "reactions",
			      aliasName => $arguments->{reactionIDaliasType},
			      alias => $arguments->{id}->[0],
			      uuid => $searchRxn->uuid()
	...
	if($result->{balanced}==0 && (defined($result->{error}) || defined($result->{imbalancedAtoms}))){
	    Bio::KBase::ObjectAPI::Util::utilities::verbose("Rejecting: ".$rxn->id()." based on status: ".$rxn->status());
	    return;
	}
    }

    Bio::KBase::ObjectAPI::Util::utilities::verbose("Creating reaction ".$rxn->uuid()." (".$arguments->{id}->[0].")");
...
Definition:
	Bio::KBase::ObjectAPI::Compound = Bio::KBase::ObjectAPI::Biochemistry->searchForCompound(string);
Description:
...
sub searchForCompound {
	...
	#Next, search by name
	if (!defined($cpdobj)) {
		my $searchname = Bio::KBase::ObjectAPI::Compound->nameToSearchname($compound);
...
Definition:
	Bio::KBase::ObjectAPI::Reaction = Bio::KBase::ObjectAPI::Biochemistry->searchForReaction(string);
Description:
...
Definition:
	Bio::KBase::ObjectAPI::Compartment = Bio::KBase::ObjectAPI::Biochemistry->searchForCompartment(string);
Description:
...
Definition:
	void mergeBiochemistry(Bio::KBase::ObjectAPI::Biochemistry,{});
Description:
...
sub mergeBiochemistry {
	...
	if($type eq "compounds" && !$opts->{consolidate} && defined($opts->{mergevia})){
    	...
    	my $uuidTranslation = {};
    	$opts->{touched}={};
	Bio::KBase::ObjectAPI::Util::utilities::verbose("Merging ".scalar(@$objs)." ".$type." from ".$bio->name()." with ".scalar(@{$self->$type()})." from ".$self->name());
    	for (my $j=0; $j < @{$objs}; $j++) {
		    my $obj = $objs->[$j];
		    my $aliases={};
			...
			Bio::KBase::ObjectAPI::Util::utilities::verbose("Duplicate ".substr($type,0,-1)." found; ".$objId." merged to ".$dupObjId);
			...
			foreach my $aliasName (keys %$aliases){
				...
				if($aliasName eq "searchname" && $self->getObjectByAlias("compounds",$alias,"searchname")){
				    Bio::KBase::ObjectAPI::Util::utilities::verbose("Skipping searchname ".$alias." as its already present");
				}else{
				    Bio::KBase::ObjectAPI::Util::utilities::verbose("Adding alias ".$alias." from ".$aliasName." for ".$dupObj->uuid()); 
			...
			if($type eq "compounds" && $dupObj->formula() eq "noformula" && $obj->formula() ne "noformula"){
			    Bio::KBase::ObjectAPI::Util::utilities::verbose("Copying over formula from $objId to $dupObjId\n");
			...
			if($type eq "compounds" && $dupObj->deltaG() == 10000000 && $obj->deltaG() ne 10000000){
			    Bio::KBase::ObjectAPI::Util::utilities::verbose("Copying over thermodynamics from $objId to $dupObjId\n");
			...
			Bio::KBase::ObjectAPI::Util::utilities::verbose("Adding new ".substr($type,0,-1)." (".$objId.") to biochemistry");
			foreach my $aliasName (keys %$aliases){
			    foreach my $alias (keys %{$aliases->{$aliasName}}){
				if($aliasName eq "searchname" && $self->getObjectByAlias("compounds",$alias,"searchname")){
				    Bio::KBase::ObjectAPI::Util::utilities::verbose("Skipping searchname ".$alias." as its already present\n");
...
Definition:
	void checkForDuplicateReactionSet(Bio::KBase::ObjectAPI::Media);
Description:
...
Definition:
	void checkForDuplicateCompoundSet(Bio::KBase::ObjectAPI::Media);
Description:
	This command checks if the input media is a duplicate for an existing media
...
Definition:
	void checkForDuplicateMedia(Bio::KBase::ObjectAPI::Media);
Description:
	This command checks if the input media is a duplicate for an existing media
...
Definition:
	void checkForDuplicateReaction(Bio::KBase::ObjectAPI::Reaction);
Description:
...
Definition:
	void checkForDuplicateCompound(Bio::KBase::ObjectAPI::Compound);
Description:
...
sub checkForDuplicateCompound {
	...
	foreach my $mergeNamespace (@{$opts->{mergevia}}){
		...
		if($dupObj && ( !exists($opts->{touched}{$dupObj->uuid()}) || defined($opts->{consolidate}) )){
		    Bio::KBase::ObjectAPI::Util::utilities::verbose("Duplicate compound found using $alias in $mergeNamespace");
		    return $dupObj;
		}
	    }
	}
...
Definition:
	void checkForDuplicateCompartment(Bio::KBase::ObjectAPI::Cue);
Description:
...
Definition:
	void checkForDuplicateCue(Bio::KBase::ObjectAPI::Cue);
Description:
...
sub __upgrade__ {
	...
	if ($version == 1) {
		return sub {
			my ($hash) = @_;
			my $bioStruct = Bio::KBase::ObjectAPI::BiochemistryStructures->new({});
